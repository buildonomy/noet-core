...
theme:
  ...
  # Footer / banner text (rendered on every page in this network)
  footer_text: "PROPRIETARY & EXPORT-CONTROLLED DATA"
  banner_text: "CUI // SP-EXPT"
```
...
pub struct ThemeConfig {
    ...
    /// Footer text rendered on every page in this network.
    /// Appears as a persistent banner at the bottom of the page,
    /// above the default "Generated by Noet" credit.
    /// Use case: export control markings, classification banners,
    /// proprietary notices.
    pub footer_text: Option<String>,

    /// Banner text rendered at the top of every page in this network.
    /// Use case: classification markings (e.g. "CUI // SP-EXPT").
...
6. **Footer/banner text rendering**: Should footer/banner text support Markdown formatting or be plain text only?
   - **Recommendation**: Plain text initially. HTML injection is a security concern for shared sites.
   - **Cascade**: Network `footer_text` should override any CLI `--footer-text` for that network's pages, or should CLI be the override? Recommend: network payload is the source of truth, CLI is a fallback for corpora that don't control their own `index.md`.

7. **Banner positioning**: Top-of-page banner (above nav) vs. inline banner (above content)?
   - **Recommendation**: Fixed banner above the entire layout (like classification markings in government systems). Visible at all times regardless of scroll position.

## Future Work (Post-Issue 25)
...
- **Per-page banner overrides**: Allow individual documents to override network-level banners

...
> features. This issue's task list will need heavy modification once
> the attestation service and inference engine (Issue 93) are
> operational. Navigation features (go-to-definition, find-references,
> document-outline) are likely to survive mostly intact; editing
> features will be reshaped by the rule map architecture.

**Priority**: MEDIUM - Enhances IDE experience for v0.3.0+
**Estimated Effort**: 5-7 days
**Dependencies**: Issue 11 (Basic LSP must be working), attestation
service (attestation_fabric.md), Issue 93 (inference engine)

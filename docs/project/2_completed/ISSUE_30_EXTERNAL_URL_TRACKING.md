...
> External URLs are already tracked as BeliefNodes via the existing `href_namespace()` /
> `push_relation()` path in `builder.rs`. The content-hashing / URL-fetching / manifest-writing
> portions were never implemented and are not planned in the near term. The opt-in per-network
> URL snooping idea has been moved to `BACKLOG.md`. No further action required on this issue.

...
> The boolean operator (`AND`, `NOT`) and field-scoped search (`title:`, `schema:`, `kind:`)
> backlog items from this issue are absorbed into Issue 70 Phase 4. The ranking boost factors
> (depth boost, cross-reference boost) remain in the BACKLOG.md. The lazy search index

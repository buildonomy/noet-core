# Issue 13: HTML Generation CLI Integration

> **Status: OBE (Overtaken By Events)**
> HTML generation via `noet parse` and `noet watch` is fully implemented. The architecture
> described here (separate `HtmlGenerator` struct, `FileUpdateSyncer` integration, `--html`
> flag wiring) was not followed as written, but the user-visible outcomes — one-shot HTML
> output and continuous watch-mode site generation with static asset handling — are

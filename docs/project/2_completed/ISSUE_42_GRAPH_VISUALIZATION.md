...
> drawer integration) carries over unchanged. The key architectural insight: the graph
> renderer shares the same row source and `currentContextMap` data as the table renderer —
> switching render modes does not reload data. No further action required on this issue.

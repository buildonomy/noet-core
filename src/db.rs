...
impl DbConnection {
    /// Resolve a repo-relative path like `"subnet/file.md"` against `net` by walking
    /// the `paths` table one segment at a time.
    ///
    /// The DB stores paths network-relative: `(net=subnet_bref, path="file.md")`.
    /// A direct `path = "subnet/file.md" AND net = repo_bref` query never matches.
    /// This helper decomposes the path at the first `/`, looks up the sub-network BID
    /// for the prefix under `net`, then recurses with the remainder — mirroring
    /// `get_all_paths`'s traversal in reverse.
    ///
    ...
    async fn resolve_net_path(
        ...
        // Direct match: path is already network-relative (no cross-network hop needed).
        ...
        if let Some((bid_str,)) = row {
            return Ok(Bid::try_from(bid_str.as_str()).ok());
        }

        // Path contains '/': the first segment names a sub-network directory registered
        // under `net`.  Look it up, then resolve the remainder within that sub-net.
        if let Some(slash) = path.find('/') {
            ...
            .fetch_optional(&self.0)
            .await?;
            if let Some((sub_bid_str,)) = sub_row {
                if let Ok(sub_bid) = Bid::try_from(sub_bid_str.as_str()) {
                    // Use Box::pin to satisfy the async recursion requirement.
                    return Box::pin(self.resolve_net_path(sub_bid.bref(), remainder)).await;
                }
            }
        }
...
fn get_all_paths(
    ...
    Box::pin(async move {
        ...
        let mut row_results = rows
            .into_iter()
            .filter_map(|(path, target, ordering)| {
                // Filter out network index files and subsections
                if path.is_empty()
                    || !include_index
                        && ordering
                            .split('.')
                            .any(|order| order.parse::<u16>() == Ok(u16::MAX))
                {
                    None
                } else {
        ...
        tracing::debug!(
            "[get_all_paths] Found {} subnets in network {}",
            row_nets.len(),
            network_bid
        );
        ...
        for new_net in new_nets.iter() {
            ...
            let mut sub_results = get_all_paths(
                pool.clone(),
                *new_net,
                include_index,
...
impl BeliefSource for DbConnection {
    ...
    async fn eval_unbalanced(&self, expr: &Expression) -> Result<BeliefGraph, BuildonomyError> {
        // Intercept cross-network NetPath queries before they hit the SQL layer.
        //
        // The DB stores paths network-relative: (net=subnet_bref, path="file.md").
        // A repo-relative path like "subnet/file.md" will never match the direct SQL
        // generated by StatePred::NetPath's AsSql impl.  resolve_net_path walks the
        // paths table one segment at a time to find the correct BID, then we rewrite
        // the expression as a plain BID lookup — which the SQL layer handles correctly.
        let rewritten: Expression;
        let effective_expr: &Expression =
            if let Expression::StateIn(StatePred::NetPath(net, path)) = expr {
                if path.contains('/') {
                    match self.resolve_net_path(*net, path).await? {
                        Some(bid) => {
                            rewritten = Expression::StateIn(StatePred::Bid(vec![bid]));
                            &rewritten
                        }
                        None => {
                            tracing::debug!(
        ...
        let mut states = self.get_states(effective_expr).await?;
        // tracing::debug!(
